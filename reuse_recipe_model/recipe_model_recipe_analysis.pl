...
recipe_edge(Rid,Pid2,Pid1,seq(RSeq1,RSeq2)) :- recipe(Rid,RSeq1,Pid1,Rprev),
...
serialize_recipe_path(Rid,Pid2,Pid1,S1,N,S2,seq(S1,S2,N)) :-
    recipe_path(Rid,Pid1,Pid2,seq(S1,S2,N)).
    
serialize_recipe_path(Rid,Pid1,Pid2,S1,N,S2,Sign) :-
    serialize_recipe_path(Rid,Pid1,Pid2,_,_,seq(S1,S2,N),Sign), N>0.

serialize_recipe_path(Rid,Pid1,Pid2,seq(S1,S2),0,-1,Sign) :-
    serialize_recipe_path(Rid,Pid1,Pid2,_,_,seq(S1,S2,N),Sign), N=0.
...
select_serialize_recipe(Rid,p1,p6,S1,N,S2,Sign) :- 
serialize_recipe_path(Rid,p1,p6,S1,N,S2,Sign).

#show select_serialize_recipe/7.
