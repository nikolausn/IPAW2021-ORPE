...
% schema with only data type (doesn't care about naming)
...
% instantiate column
column(c11,a3,cs11,0).
column(c12,a3,cs12,c11).
column(c13,a3,cs13,c12).

column(c14,a4,cs14,0).
column(c15,a4,cs15,c14).
...
step_input_schema(column_state(CIid,SIid),Pid,column_schema(CSIid,Key,Value)) :-
    workflow_step(column_state(CIid,SIid),Pid,column_state(COid,SOid)),
...
workflow_input_violation(column_state(CIid,SIid),Pid,column_schema(CSPid,Key,Value)) :-
    ...
    workflow_step(column_state(CIid,SIid),Pid,_),
...
step_output_schema(column_state(COid,SOid),Pid,column_schema(CSIid,Key,Value)) :-
    workflow_step(column_state(CIid,SIid),Pid,column_state(COid,SOid)),
...
workflow_output_violation(column_state(CIid,SIid),Pid,column_schema(CSPid,Key,Value)) :-
    ...
    column_schema(CSPid,Key,Value),
    workflow_step(column_state(CIid,SIid),Pid,column_state(COid,SOid)),
    not step_output_schema(column_state(COid,SOid),Pid,column_schema(_,Key,Value)).
...
% probably: step?
workflow_step(column_state(c1,1),p1,column_state(c1,2)).
workflow_step(column_state(c1,1),p1,column_state(c4,1)).
workflow_step(column_state(c1,1),p1,column_state(c5,1)).
workflow_step(column_state(c1,2),p2,column_state(c1,3)).
workflow_step(column_state(c4,1),p2,column_state(c4,2)).
workflow_step(column_state(c5,1),p2,column_state(c5,2)).
workflow_step(column_state(c4,2),p4,column_state(c7,1)).
workflow_step(column_state(c5,2),p4,column_state(c7,1)).
workflow_step(column_state(c1,3),p4,column_state(c1,4)).
workflow_step(column_state(c10,1),p5,column_state(c10,2)).
workflow_step(column_state(c10,2),p6,column_state(c10,3)).
workflow_step(column_state(c11,1),p1,column_state(c11,2)).
workflow_step(column_state(c11,2),p2,column_state(c11,3)).
...
workflow_input(P,column_state(CIId,CIState),column_state(COId,COState),CIName,COName) :- 
    workflow_step(column_state(CIId,CIState),P,column_state(COId,COState)),
    column(CIId,_,CSIId,_),
...
% workflow graph
workflow_path(Source,Target,2,cons(Source,2,cons(Target,1,empty))) :- workflow_step(Source,_,Target).
workflow_path(Source,Target,NN+1,cons(Source,NN+1,L)) :- workflow_step(Source,_,Temp), workflow_path(Temp,Target,NN,L).
