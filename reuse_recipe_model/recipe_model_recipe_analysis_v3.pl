...
% remark these two lines for violation example
% removing the original column cs2/cs3
output_process(p4,2,cs2).
output_process(p4,3,cs3).
...
process_edge(CSin,Pid) :- input_process(Pid,_,CSin).
process_edge(Pid,CSout) :- output_process(Pid,_,CSout).
...
select_serialize_derived_input(Rid,A,B,seq(S1,S2),M-1,Sign,N,CSin) :-
...
select_serialize_derived_input(Rid,A,B,seq(S1,S2),M-1,Sign,N,CSout) :-
    select_serialize_recipe(Rid,A,B,seq(S1,S2),M,Sign,N),
    recipe(Rid,S1,P1,_),
    M=0,
    output_process(P1,_,CSout). 

select_serialize_derived_input(Rid,A1,B1,seq(S1,S2),M1,Sign,N1,CSin) :-
    %select_serialize_recipe(Rid,A,B,seq(S1,S2),M,Sign,N),
    %recipe(Rid,S2,P1,_),
    %M=0,
    %output_process(P1,_,CSout).     
    select_serialize_recipe(Rid,A1,B1,seq(S1,S2),M1,Sign,N1),
...
select_recipe_input_violation(Rid,A,B,seq(S1,S2),M,Sign,N,P1,CSin) :-
    select_serialize_recipe(Rid,A,B,seq(S1,S2),M,Sign,N),
    recipe(Rid,S2,P1,_),
    M>0,
    input_process(P1,_,CSin),
    not select_serialize_derived_input(Rid,A,B,seq(_,S1),M-1,Sign,_,CSin).
...
select_recipe_input_violation(Rid,A,B,seq(S1,S2),M,Sign,N,P1,CSin) :-
    ...
    M=0,
    input_process(P1,_,CSin),
    not select_serialize_derived_input(Rid,A,B,seq(S1,S2),M-1,Sign,_,CSin).    
...
%#show select_serialize_derived_input_temp/5.
%#show all_recipe_input/2.
